...
@if(Route::is('dists.list'))
    <script>
            //dynamic featured data
function featured(id,type,url,divId) {  
   // alert(divId);
    $.ajaxSetup({
      headers: {
   ...
   var featured = ($('#'+divId).is(":checked"))?1:0;

    $.ajax({
      url: url, // this is the object instantiated in wp_localize_script function
      type: 'POST',
      data:{id:id,type:type,featured:featured},
       dataType: 'json',
      success: function( data ){        
        console.log(data);
       // $('#'+divId).prop("checked", !$('#'+divId).prop("checked"));
      },
      error: function (msg) {
      console.log(msg);
    ...
    success: function( data ){
     ...
     var  APP_URL = '{{url("/")}}/';
      ...
      $.each(data, function(index, item) {
        //alert(APP_URL+item.logo);
        var india = "";
        if(data.location.length != 'distributor'){
          india ="<br><b>Across India</b><br> "+data.location;
        }

        var world = "";
        if(data.location2.length != 'distributor'){
        ...
        if(item.mode == 'appoint' && type == 'distributor')
        {
        ...
        html.append(
          '<tr><th>Logo</th><td><img src="'+APP_URL+item.logo+'" class="img-responsive img-thumbnail" width="200"/></td>'+
       ...
       else if(item.mode == 'become' && type == 'distributor')
        {
        ...
        else if(item.mode == 'become' && type == 'frenchies')
        {
        ...
        else if(item.mode == 'appoint' && type == 'frenchies')
        {
          var brand = (item.brand != null)?item.brand:" - ";  
        ...
        html.append(
          ...
          '<th>Remark</th><td>'+item.remark+'</td></tr>' 
             ...
             //sales agent 
        else if(item.mode == 'become' && type == 'sales')
        {
        ...
        else if(item.mode == 'appoint' && type == 'sales')
        {      
